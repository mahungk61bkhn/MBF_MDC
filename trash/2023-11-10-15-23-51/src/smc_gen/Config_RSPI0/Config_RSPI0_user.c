...
#pragma interrupt r_Config_RSPI0_error_interrupt(vect=VECT(RSPI0,SPEI0),fint)
#else
#pragma interrupt r_Config_RSPI0_error_interrupt(vect=VECT(RSPI0,SPEI0))
#endif
static void r_Config_RSPI0_error_interrupt(void)
{
    uint8_t err_type;
...
#pragma interrupt r_Config_RSPI0_idle_interrupt(vect=VECT(RSPI0,SPII0),fint)
#else
#pragma interrupt r_Config_RSPI0_idle_interrupt(vect=VECT(RSPI0,SPII0))
#endif
static void r_Config_RSPI0_idle_interrupt(void)
{
    /* Disable RSPI function */
    RSPI0.SPCR.BIT.SPE = 0U;
...
void SPI_Send_Receive(uint16_t * const tx_buf, uint16_t tx_num, uint16_t * const rx_buf)
{
	 r_spi0_txdone = 0U;
