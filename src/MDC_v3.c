...
void main(void)
{
	MDC_regs[21] = 0; // Charge Time
	MDC_regs[22] = 0; // Discharge Time
	...
	if (charge_time_count > 900000)
		charge_time_count = 0;
	if (discharge_time_count > 900000)
		discharge_time_count = 0;
...
// thanh test===============================
		...
		//		//end_print_test
		//		//print test
		//		sprintf(print_str,"\r\nCHECK 1: %d 6: %d 28: %d 29: %d 30: %d 31: %d",MDC_regs[1],MDC_regs[6],MDC_regs[28],MDC_regs[29],MDC_regs[30],MDC_regs[31]);
		//		RS485_M_Ctr = 1U; //RS485 send mode
		//		R_SCI1_AsyncTransmit((uint8_t*)print_str,strlen(print_str));
		//		RS485_M_Ctr = 0U; //RS485 receive mode
		//		memset(print_str, 0, sizeof(print_str));
		//		//end_print_test
		//		//print test
		//		sprintf(print_str,"\r\nCHECK 2: %d 7: %d 33: %d 34: %d 35: %d 36: %d",MDC_regs[2],MDC_regs[7],MDC_regs[33],MDC_regs[34],MDC_regs[35],MDC_regs[36]);
		//		RS485_M_Ctr = 1U; //RS485 send mode
		//		R_SCI1_AsyncTransmit((uint8_t*)print_str,strlen(print_str));
		//		RS485_M_Ctr = 0U; //RS485 receive mode
		//		memset(print_str, 0, sizeof(print_str));
		//		//end_print_test
		//		//print test
		//		sprintf(print_str,"\r\nCHECK 3: %d 8: %d 10: %d 15: %d 48: %d 49: %d",MDC_regs[3],MDC_regs[8],MDC_regs[10],MDC_regs[15],MDC_regs[48],MDC_regs[49]);
		//		RS485_M_Ctr = 1U; //RS485 send mode
		//		R_SCI1_AsyncTransmit((uint8_t*)print_str,strlen(print_str));
		//		RS485_M_Ctr = 0U; //RS485 receive mode
		//		memset(print_str, 0, sizeof(print_str));
...
void main_FW_update(void) {
	...
	if (FW_UP_SUCCESS != ret_fw_up) {
		...
		switch (ret_fw_up) {
			case FW_UP_ERR_NOT_OPEN: // fw_up_open_flash has not been run
				MCU_LED_STT = 0;
				break;
			case FW_UP_ERR_INVALID_RESETVECT: // The value of reset vector of the firmware is invalid
				MCU_LED_PSU = 0;
				break;
			case FW_UP_ERR_SWITCH_AREA: // Flash API(switch start-up area) error
				MCU_LED_MCC = 0;
				break;
			default:
				/** Do nothing */
	...
	if (rms_current >= 1) //Sensor is plugged in
	{
		if (rms_current > Vref) //Discharge Current
		{
			...
			if (channel == 1)
			{
				//CALIB theo điện áp
				if (rms_current < 8.5)
					rms_current = 0.9328 * (0.9995 * (0.996 * rms_current - 1.1422) + 0.002108) + 0.41139;
				else if (rms_current < 19 && rms_current > 8.5)
					rms_current = 1.0481 * (0.9287591684 * rms_current - 0.54253252992) - 0.27373;
				else if (rms_current < 26.5 && rms_current > 19)
					rms_current = 1.0122 * rms_current - 1.21456;
				else
					rms_current = 0.9799 * rms_current - 0.8912;
				// CALIB THEO SENSOR
...
//					else  rms_current = 0.9799*rms_current - 0.4712;
			...
			{
				if (rms_current < 8.5)
					rms_current = 1.0255 * (0.9982 * (0.9479 * rms_current - 0.66263) + 0.002926) - 0.13522;
				else if (rms_current < 19 && rms_current > 8.5)
					rms_current = 1.0085 * (0.9888763225 * rms_current - 0.40057670525) - 0.43182;
				else if (rms_current < 26.5 && rms_current > 19)
					rms_current = 0.9906 * rms_current - 0.83595;
				else
					rms_current = 1.0055 * rms_current - 1.1241;
				//CALIB THEO SENSOR
...
//					else  rms_current = 1.0055*rms_current - 1.1241;
			...
			{
				if (rms_current < 8.5)
					rms_current = 0.964 * (0.9999 * (0.9868 * rms_current - 1.0048) - 0.002822) + 0.23661;
				else if (rms_current < 19 && rms_current > 8.5)
					rms_current = 0.9997 * (1.00977598462 * rms_current - 0.45463268081) - 0.70955;
				else if (rms_current < 26.5 && rms_current > 19)
					rms_current = 1.0154 * rms_current - 1.34914;
				else
					rms_current = 0.9846 * rms_current - 0.79;
				//CALIB THEO SENSOR
...
//					else  rms_current = 1.0055*rms_current - 1.1241;
			...
			{
				if (rms_current < 8.5)
					rms_current = 1.01387 * (0.9958 * (0.9465 * rms_current - 0.66626) + 0.005483) - 0.075593;
				else if (rms_current < 19 && rms_current > 8.5)
					rms_current = 1.0048 * (0.951848277 * rms_current - 0.712869431) + 0.38113;
				else if (rms_current < 26.5 && rms_current > 19)
					rms_current = 1.0059 * rms_current - 1.20233;
				else
					rms_current = 1.0027 * rms_current - 1.035;
				//CALIB THEO SENSOR
